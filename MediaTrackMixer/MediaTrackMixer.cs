...
namespace MediaTrackMixer
{
    public class MediaTrackMixer(string ffmpegPath)
    {
        ...
        public async Task Mix(List<TrackGroup> tracks, string output, List<Map> maps, IProgress<double>? progress = null)
        {
            var inputArgs = string.Join(" ", tracks.Select(tr => $"-i \"{tr.Path}\""));
            var mapTrackArgs = string.Join(" ", maps.Select(mp => mp.ForChapter ? $"-map_chapters {mp.InputIndex}" : $"-map {mp.InputIndex}:{mp.TrackIndex}"));
            var i = 0;
            var mapTitleArgs = string.Join(" ", maps.Select(mp => {
                var arg = string.IsNullOrWhiteSpace(mp.NewTitle) ? null
                    : $"-metadata:{(mp.ForChapter ? "c" : "s")}:{i} title=\"{mp.NewTitle}\"";
                i++;
                return arg;
            }).Where(arg => arg != null));
        ...
        public class Map(int inputIndex, int trackIndex, bool forChapter = false, string? newTitle = null)
        {
